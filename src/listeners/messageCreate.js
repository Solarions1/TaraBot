					...
					return await message.channel.send({
						embeds: [
							new MessageEmbed()
								...
								.setFooter("Ticket by Manu", message.guild.iconURL())
						],
						files: [attachment]
					});
				}
				...
				if (tickets.count >= cat_row.max_per_member) {
					if (cat_row.max_per_member === 1) {
						const embed = new MessageEmbed()
							...
							.setFooter(this.client.utils.footer("Ticket by Manu", i18n('message_will_be_deleted_in', 15)), message.guild.iconURL());
						try {
						...
						const embed = new MessageEmbed()
							...
							.setFooter(this.client.utils.footer("Ticket by Manu", i18n('message_will_be_deleted_in', 15)), message.author.iconURL());
						try {
							response = await message.author.send({ embeds: [embed] });
						} catch {
							response = await message.channel.send({ embeds: [embed] });
